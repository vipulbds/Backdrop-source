...
class HeaderComponent extends Component {
  ...
  /**
   * Width of the HEADER (not the window) at the moment the drawer took over.
   *
   * This used to store `window.innerWidth`, which is the wrong dimension and
   * is why the header stayed stuck in drawer mode after the cart drawer was
   * closed: opening the drawer squeezes the page, the squeezed menu reports
   * minimum-reached, and we recorded the *window* width — which the squeeze
   * never changes. Closing the drawer therefore compared 1512 > 1512, the
   * test failed, and the desktop menu never came back.
   *
   * Measuring the header itself means anything that takes room away from the
   * menu without touching the window — the drawer squeeze, the scrollbar
   * appearing, a longer cart pill — is seen, and is reverted when the room
   * comes back.
   *
   * @type {number | null}
   */
  ...
  /**
   * Pending re-check after a modal closes
   * @type {number | null}
   */
  #recheckTimeout = null;
  ...
  #resizeObserver = new ResizeObserver(([entry]) => {
    ...
    // Compared against the header's own inline size — see #menuDrawerHiddenWidth.
    if (
      this.#menuDrawerHiddenWidth != null &&
      Math.round(entry.borderBoxSize[0].inlineSize) > this.#menuDrawerHiddenWidth
    ) {
  ...
  /**
   * A modal closing can hand the menu its room back without the header ever
   * changing size — the cart drawer squeeze and the scrollbar both do that,
   * and neither fires the ResizeObserver above. So when we are in the forced
   * drawer state, re-measure shortly after any dialog closes.
   *
   * Re-measuring is safe: #updateMenuVisibility(false) clears minimum-reached
   * and lets overflow-list measure again. If the menu genuinely still doesn't
   * fit, it re-fires overflowMinimum and we land back in drawer mode.
   *
   * `close` does not bubble, hence the capture-phase listener on document.
   */
  #handleModalClose = () => {
    if (this.#menuDrawerHiddenWidth == null || this.#recheckTimeout != null) return;

    // Long enough for the drawer's closing animation to release the squeeze.
    this.#recheckTimeout = setTimeout(() => {
      this.#recheckTimeout = null;
      if (this.#menuDrawerHiddenWidth != null) this.#updateMenuVisibility(false);
    }, 350);
  };
  ...
  #updateMenuVisibility(hideMenu) {
    if (hideMenu) {
      this.#menuDrawerHiddenWidth = Math.round(this.getBoundingClientRect().width);
    } else {
      this.#menuDrawerHiddenWidth = null;
  ...
  connectedCallback() {
    ...
    document.addEventListener('close', this.#handleModalClose, true);
    document.addEventListener('cart:update', this.#handleModalClose);
  ...
  disconnectedCallback() {
    ...
    document.removeEventListener('close', this.#handleModalClose, true);
    document.removeEventListener('cart:update', this.#handleModalClose);
    if (this.#recheckTimeout != null) {
