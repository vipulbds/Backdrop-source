...
var CONFIG = {
  ...
  // TRUE puts BLENDED and PAID SUBTOTAL at the TOP of each country table, above
  // the per-channel rows. A total normally belongs at the foot, but here the
  // channel rows are the ones attribution can leave empty, and burying the one
  // complete line under five blank ones made the tab read as broken — the Exec
  // Summary showed figures while the country tab "showed nothing", when in fact
  // both held exactly the same BLENDED numbers (2026-08-22). Leading with the line
  // that is always complete fixes that at a glance. FALSE restores totals-at-foot.
  TOTALS_FIRST: true,
...
/* Distinct-count, used only by selfTest to prove no line is emitted twice. */
function new_Set_size_(arr) {
  var seen = {}, n = 0;
  (arr || []).forEach(function (v) { if (!seen[v]) { seen[v] = 1; n++; } });
  return n;
}
...
function vetGrainRow_(o, dk, code, ch, bad) {
  // Spend and attribution reach the grain from DIFFERENT owners — spend from the
  // Ad Budget Tracker, revenue from bds-unified — so one being impossible says
  // nothing about the other. Zeroing both (the first version of this) threw away
  // perfectly good spend and could silently blank a channel's whole period.
  var badSpend = overCap_(o.spend);
  var badAttr  = overCap_(o.revenue) || overCap_(o.newRevenue);
  if (!badSpend && !badAttr) return false;

  bad.push({ code: code, day: dk, kind: 'impossible', tab: CONFIG.GEO_TABS.GRAIN,
    detail: ch + ': ' + (badSpend ? 'Spend ' + fmtBig_(o.spend) : 'Revenue ' + fmtBig_(o.revenue) +
            ' · New ' + fmtBig_(o.newRevenue)) +
            '  (only the ' + (badSpend ? 'spend' : 'attribution') + ' half was discarded)' });

  if (badSpend) o.spend = 0;
  if (badAttr) {
    // The COUNTS go with the money on purpose. Zeroing revenue while leaving
    // orders > 0 would make hasData true and print a 0.00x ROAS — a false RED
    // asserting the channel sold nothing. Dropping the counts too makes the line
    // read ⚪ "cannot measure", which is what actually happened.
    o.revenue = 0; o.newRevenue = 0; o.orders = 0; o.matched = 0;
  }
  return true;
}
...
function linesFor_(code, aCur, aPrior) {
  var channels = CONFIG.CHANNELS.slice();
  if (CONFIG.SHOW_OTHER) {
    var extra = {};
    [aCur, aPrior].forEach(function (a) {
      var b = a.byEntity[code]; if (!b) return;
      Object.keys(b.channels).forEach(function (ch) { if (channels.indexOf(ch) === -1) extra[ch] = 1; });
    });
    Object.keys(extra).sort().forEach(function (ch) { channels.push(ch); });
  }
  var totals = [];
  totals.push('OVERALL');                                    // printed as BLENDED
  if (CONFIG.SHOW_PAID_SUBTOTAL) totals.push('PAID SUBTOTAL');

  // BLENDED first: it is Shopify ground truth, so it is the one line that is
  // always complete. The channel rows below it depend on attribution and can be
  // legitimately empty — putting them first made the whole tab look empty.
  if (CONFIG.TOTALS_FIRST) return totals.concat(channels);
  return channels.concat(totals.slice().reverse());          // PAID then BLENDED, at the foot
}
...
function money_(v, hasData) {
  // The three states have to LOOK like three states. Before this, a measured zero
  // printed blank while the ROAS built from the same zero printed "0.00x" — the
  // two cells of one row contradicting each other, which is exactly what made a
  // real tab unreadable (2026-08-21: "New Cx Revenue shows nothing but New-Revenue
  // ROAS shows 0.00x").
  //   hasData === false -> blank. We could not measure it; matches the ⚪ flag.
  //   hasData === true  -> a number, INCLUDING 0.00. We measured it; that is a fact.
  //   hasData omitted   -> blank for zero. Used for spend, where 0 means "not
  //                        running" and a column of $0.00 would be noise.
  if (hasData === false) return '';
  var n = num_(v);
  if (n) return round2_(n);
  return hasData === true ? 0 : '';
}
...
function buildEntityTab_(e, per, src, aCur, aPrior, health) {
  ...
  var warn = entityWarnings_(e, aCur, src, aPrior, per), legend = legendText_(per, src, warn);
  ...
  lines.forEach(function (line) {
    ...
    var r = {
      ...
      newP: money_(p.newRev, p.newHasData), newC: money_(c.newRev, c.newHasData),
      ...
      ovP: money_(p.rev, p.hasData), ovC: money_(c.rev, c.hasData),
...
function entityWarnings_(e, aCur, src, aPrior, per) {
  var out = [], b = aCur.byEntity[e.code];
  if (!b) return out;

  /* A channel that HAD attributed orders last period and has NONE this period has
     not fallen to zero — it has stopped being measurable, and its current-period
     cells are therefore EMPTY. That empty column is the single most confusing
     thing this report can show (2026-08-21: "revenue and roas and blended show
     only 1-21 july not for the 1-21 aug"), so it gets named in full rather than
     left for the reader to infer from a blank. */
  var stopped = [];
  var pb = (aPrior && aPrior.byEntity) ? aPrior.byEntity[e.code] : null;
  if (pb && per) {
    CONFIG.CHANNELS.forEach(function (ch) {
      var cm = b.channels[ch], pm = pb.channels[ch];
      var had = pm && (pm.orders > 0 || pm.revenue > 0);
      var has = cm && (cm.orders > 0 || cm.revenue > 0);
      if (had && !has) stopped.push(ch);
    });
  }
  if (stopped.length) {
    out.push('WHY THE ' + per.cur.short.toUpperCase() + ' COLUMNS ARE EMPTY FOR ' +
      stopped.join(', ') + ': those channels had attributed orders in ' + per.prior.label +
      ' and have NONE in ' + per.cur.label + '. So their New Cx Revenue, Blended Revenue and both ROAS ' +
      'cells for ' + per.cur.short + ' are BLANK, not zero — blank means we could not measure it. The ' +
      'money itself is NOT missing: it is in the OVERALL line, which comes from Shopify. What stopped is ' +
      'the CHANNEL LABEL on those orders. Fix upstream — run debugSources() in the GEO project and look ' +
      'at what bds-unified is writing into Source; if it is blank or unrecognised, every order lands in ' +
      'Direct and no paid channel can be graded.');
  }
  ...
  CONFIG.ENTITIES.forEach(function (e) {
    ...
    bases.forEach(function (bs) {
      ...
      var r = {
        ...
        spendP: money_(p.spend), spendC: money_(c.spend),
        newP: money_(p.newRev, p.newHasData), newC: money_(c.newRev, c.newHasData), newG: growthVal_(p.newRev, c.newRev, c.newHasData),
        newGR: ragGrowth_(p.newRev, c.newRev, hasSpend, c.newHasData || p.newHasData, c.newHasData),
        rnP: roasVal_(p.spend, p.newRev, p.newHasData), rnC: roasVal_(c.spend, c.newRev, c.newHasData),
        rnR: ragRoas_(c.spend, c.newRev, c.newHasData, CONFIG.TARGETS.roasNew),
        ovP: money_(p.rev, p.hasData), ovC: money_(c.rev, c.hasData), ovG: growthVal_(p.rev, c.rev, c.hasData),
        ovGR: ragGrowth_(p.rev, c.rev, hasSpend, c.hasData || p.hasData, c.hasData),
        roP: roasVal_(p.spend, p.rev, p.hasData), roC: roasVal_(c.spend, c.rev, c.hasData),
        roR: ragRoas_(c.spend, c.rev, c.hasData, CONFIG.TARGETS.roasOverall),
...
function debugAttribution() {
  ...
  var src = loadSource_();
  var a = aggregate_(src, per.cur.from, per.cur.to);
  // BOTH windows, side by side. A channel with numbers in one column and nothing
  // in the other is the whole question this diagnostic exists to answer, and it is
  // invisible if you only print the current period.
  var aP = aggregate_(src, per.prior.from, per.prior.to);
  var lines = ['=== ' + VERSION + ' attribution check ===',
               '  PRIOR   = ' + per.prior.label,
               '  CURRENT = ' + per.cur.label,
               '  (orders / attributed revenue, per channel, in ' + CONFIG.REPORT_CURRENCY + ')'];
  CONFIG.ENTITIES.forEach(function (e) {
    var b = a.byEntity[e.code], pb = aP.byEntity[e.code];
    if (!b) return;
    lines.push('');
    lines.push(e.label + ' (' + e.code + ')');
    var names = {};
    Object.keys(b.channels).forEach(function (ch) { names[ch] = 1; });
    if (pb) Object.keys(pb.channels).forEach(function (ch) { names[ch] = 1; });
    var chans = Object.keys(names).sort();
    if (!chans.length) lines.push('  no rows at all in either window');
    chans.forEach(function (ch) {
      var m = b.channels[ch] || blank_(), pm = (pb && pb.channels[ch]) || blank_();
      var note = '';
      if ((pm.orders > 0 || pm.revenue > 0) && !(m.orders > 0 || m.revenue > 0)) {
        note = '   <-- ATTRIBUTION STOPPED: it had orders in the prior window and none now, which is ' +
               'why the CURRENT cells are blank';
      } else if (m.spend > 0 && m.orders === 0) {
        note = '   <-- SPEND BUT NO ORDERS in either window: this is why it shows ⚪';
      }
      lines.push('  ' + ch +
        ':  prior ' + pm.orders + ' order(s) / ' + round2_(pm.revenue) +
        '   ->   current ' + m.orders + ' order(s) / ' + round2_(m.revenue) +
        '   (current spend ' + round2_(m.spend) + ')' + note);
    });
    var cov = b.shopify.revenue > 0 ? Math.round(1000 * b.all.revenue / b.shopify.revenue) / 10 : '';
    lines.push('  Shopify (current): ' + b.shopify.orders + ' order(s), revenue ' +
      round2_(b.shopify.revenue) + ' · attributed total ' + round2_(b.all.revenue) +
      (cov === '' ? '' : ' · coverage ' + cov + '%'));
    lines.push('  -> the gap between those two is revenue arriving with NO channel on it.');
  });
  lines.push('');
...
function selfTest() {
  ...
  /* --- "why is the August column empty" must be ANSWERED on the tab, using the
         real shape from Canada on 2026-08-21: Google had 1,112 of attributed
         revenue in 1–21 Jul and nothing at all in 1–21 Aug. --- */
  (function () {
    function ent(rev, orders) {
      return { byEntity: { CA: {
        channels: { Google: { spend: 15000, orders: orders, revenue: rev, newRevenue: 0, matched: orders } },
        paid: blank_(), all: { spend: 15000, orders: orders, revenue: rev, newRevenue: 0, matched: orders },
        shopify: { orders: 300, revenue: 136167, newRev: 91883, retRev: 44284,
                   blend: 136167, days: 21, qlDays: 21 } } } };
    }
    var perCA = periodPair_('Month to date', '2026-08-21');
    var w = entityWarnings_({ code: 'CA', label: 'Canada', currency: 'USD' },
                            ent(0, 0), { curByEntity: { CA: { USD: 1 } } }, ent(1112.4, 3), perCA);
    var j = w.join(' | ');
    eq(/WHY THE .* COLUMNS ARE EMPTY FOR Google/.test(j), true,
       'the empty current column is explained by name, not left as a blank to puzzle over');
    eq(/BLANK, not zero/.test(j), true, 'and blank is distinguished from zero in words');
    eq(/1–21 Jul/.test(j) && /1–21 Aug/.test(j), true, 'naming BOTH windows so the change is locatable');
    eq(/is in the OVERALL line/.test(j), true, 'reassuring that the money itself is not missing');
    eq(/debugSources/.test(j), true, 'and pointing at the upstream fix');
    // The reverse case must NOT warn: a channel with attribution in both windows.
    var w2 = entityWarnings_({ code: 'CA', label: 'Canada', currency: 'USD' },
                              ent(2000, 5), { curByEntity: { CA: { USD: 1 } } }, ent(1112.4, 3), perCA);
    eq(/COLUMNS ARE EMPTY/.test(w2.join(' | ')), false, 'a channel still reporting is not warned about');
    // And a three-argument caller must still work, without the new warning.
    var w3 = entityWarnings_({ code: 'CA', label: 'Canada', currency: 'USD' },
                              ent(0, 0), { curByEntity: { CA: { USD: 1 } } });
    eq(/COLUMNS ARE EMPTY/.test(w3.join(' | ')), false,
       'the prior window is optional — an old three-arg call cannot crash a build');
  })();

  /* --- BLENDED leads the country table. The user opened a country tab, saw the
         five attribution-dependent channel rows at the top with empty August
         cells, and reported the tab as broken — while the BLENDED row at the FOOT
         held exactly the figures the Exec Summary was showing (2026-08-22). --- */
  (function () {
    var emptyAgg = { byEntity: { UK: { channels: { Google: blank_(), Direct: blank_() },
      paid: blank_(), all: blank_(),
      shopify: { orders: 0, revenue: 0, newRev: 0, retRev: 0, blend: 0, days: 0, qlDays: 0 } } } };
    var savedTF = CONFIG.TOTALS_FIRST;
    CONFIG.TOTALS_FIRST = true;
    var L = linesFor_('UK', emptyAgg, emptyAgg);
    eq(L[0], 'OVERALL', 'the complete line comes FIRST, so the tab never reads as empty');
    eq(lineLabel_(L[0]), 'BLENDED', 'and prints as BLENDED');
    eq(L[1], 'PAID SUBTOTAL', 'then the paid subtotal');
    eq(L.indexOf('Google') > 1, true, 'with the per-channel detail below them');
    eq(L.length, new_Set_size_(L), 'no line appears twice');
    eq(L.indexOf('OVERALL'), L.lastIndexOf('OVERALL'), 'OVERALL is emitted exactly once');
    CONFIG.TOTALS_FIRST = false;
    var L2 = linesFor_('UK', emptyAgg, emptyAgg);
    eq(L2[L2.length - 1], 'OVERALL', 'with the flag off, BLENDED returns to the foot');
    eq(L2[L2.length - 2], 'PAID SUBTOTAL', 'and PAID sits just above it');
    eq(L2.length, L.length, 'the same lines either way — only the order changes');
    CONFIG.TOTALS_FIRST = savedTF;
  })();

  /* --- a measured zero and an unmeasurable one must not look alike --- */
  eq(money_(0, true), 0, 'a MEASURED zero prints 0.00, matching the 0.00x its own ROAS shows');
  eq(money_(0, false), '', 'an UNMEASURABLE zero stays blank, matching its ⚪');
  eq(money_(0), '', 'with no opinion given (spend), zero is still blank');
  eq(money_(1112.4, true), 1112.4, 'a real figure is unaffected');
  eq(money_(1112.4, false), '', 'nothing is printed for a line we could not measure');

  /* --- quarantining a grain row keeps the half that is still good --- */
  var gq = [];
  var halfBad = { spend: 15057.16, orders: 4, revenue: 9.9e15, newRevenue: 0, matched: 4 };
  eq(vetGrainRow_(halfBad, '2026-08-01', 'CA', 'Google', gq), true, 'the impossible half is caught');
  eq(halfBad.spend, 15057.16, 'the SPEND survives — it has a different owner and was never in doubt');
  eq(halfBad.revenue, 0, 'the impossible revenue is discarded');
  eq(halfBad.orders, 0, 'and its order count goes with it, so the line reads ⚪ not a false 0.00x red');
  var spendBad = { spend: 9.9e15, orders: 4, revenue: 5000, newRevenue: 2000, matched: 4 };
  eq(vetGrainRow_(spendBad, '2026-08-01', 'CA', 'Google', []), true, 'an impossible spend is caught');
  eq(spendBad.spend, 0, 'the spend is discarded');
  eq(spendBad.revenue, 5000, 'and the perfectly good attribution is KEPT');
  eq(spendBad.orders, 4, 'including its order count');
